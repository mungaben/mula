...
const DropdownUser = () => {
  ...
  console.log("data paseed from profile",data);
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      {dropdownOpen && (
        ...
        >
          <div className="flex items-center gap-2.5 px-5 pb-5.5 pt-3.5">
            ...
            <span className="block">
              ...
              <span className="block font-medium text-dark-5 dark:text-dark-6">
                {isLoading ? 'Loading...' : profile?.phone}
              </span>
              <span className="block font-medium text-dark-5 dark:text-dark-6">
                {isLoading ? 'Loading...' : profile?.email}
              </span>
              <span className="block font-medium text-dark dark:text-white">
                {isLoading ? 'Loading...' :`ref ${profile?.referrals.length}` }
              </span>
          ...
          <div className="p-2.5">
            <Button onClick={()=>signOut({ callbackUrl: '/' })} className="flex w-full items-center gap-2.5 rounded-[7px] p-2.5 text-sm font-medium text-dark-4 duration-300 ease-in-out hover:bg-gray-2 hover:text-dark dark:text-dark-6 dark:hover:bg-dark-3 dark:hover:text-white lg:text-base">
              <svg
                className="fill-current"
                width="18"
                height="18"
